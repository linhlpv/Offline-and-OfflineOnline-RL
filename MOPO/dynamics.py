...
class EnsembleDynamics(BaseDynamics):
    def __init__(self, model: nn.Module, optim: torch.optim.Optimizer, scaler: StandardScaler, terminal_fn: Callable[[np.ndarray, np.ndarray, np.ndarray], np.ndarray], penalty_coef: float = 0.0, uncertainty_mode: str = "aleatoric") -> None:
    ...
    def step(self, obs: np.ndarray, action: np.ndarray) -> Tuple[np.ndarray, np.ndarray, np.ndarray, Dict]:
        ...
        ensemble_samples = (mean + np.random.normal(size=mean.shape) * std).astype(np.float32) # we note that reward function is assumed to be deterministic

        # choose one model from ensemble 
        num_models, batch_size, _ = ensemble_samples.shape
        model_idxs = self.model.random_elite_idxs(batch_size) # sellect randomly samples from elites pool
        ...
        if self._penalty_coef:
            if self._uncertainty_mode == "aleatoric":
                penalty = np.max(np.linalg.norm(std, axis=2), axis=0)
    ...
    @torch.no_grad()
    def sample_next_obses(self, obs: torch.Tensor, action: torch.Tensor, num_samples: int) -> torch.Tensor:
    ...
    def format_samples_for_training(self, data:Dict) -> Tuple[np.ndarray, np.ndarray]:
        """ Prepare data for training ensemble dynamics model
        """
    ...
    def train(self, data: Dict, logger: Logger, max_epochs: Optional[float] = None, max_epochs_since_update: int= 5, batch_size: int=256, holdout_ratio: float=0.2, logvar_loss_coef: float=0.01) -> None:
        ...
        while True:
            ...
            # TODO: Write logger here to log train_loss, holdout_loss
            logger.add_scalar("dynamics_train_loss", train_loss, epoch)
            logger.add_scalar("dynamics_holdout_loss", holdout_loss, epoch)            
        ...
        self.model.load_save(logger.model_dir)
        self.model.eval()

    
    def learn(self, inputs: np.ndarray, targets:np.ndarray, batch_size: int=256, logvar_loss_coef: float=0.01) -> float:
        self.model.train()
