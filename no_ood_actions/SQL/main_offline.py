...
from sql_origin import SparseQLearning
...
def train(config: TrainConfig):
    ...
    q_network = DoubleQNetwork(state_dim, action_dim, layer_norm=config.layer_norm).to(config.device)
    v_network = ValueNetwork(state_dim, layer_norm=config.layer_norm).to(config.device)
    ...
    # Initialize actor
    trainer = SparseQLearning(**kwargs)
