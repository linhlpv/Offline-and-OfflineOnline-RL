...
from utils import soft_update, TensorBatch, ENVS_WITH_GOAL, EXP_ADV_MAX, ADV_MAX
...
class SparseQLearning:
    ...
    def _update_v(self, observations, actions, log_dict):
        ...
        sparse_term = adv / (2 * self.alpha) + 1.0
        weight = torch.where(sparse_term > 0, 1.0, 0.0).detach()
        v_loss = (weight * sparse_term.pow(2)  + v / self.alpha).mean()
    ...
    def _update_q(
        ...
        # print(f"obs diff: {(next_observations - observations).mean()}")
        with torch.no_grad():
            next_v = self.vf(next_observations)
    ...
    def _update_policy(
        ...
        v = self.vf(observations)
        weight = target_q - v.detach()
        weight = weight.clamp(0.0, ADV_MAX).detach()
        # weight *= torch.where(weight > 0, 1.0, 0.0).detach()
        
        policy_out = self.actor(observations)
        log_prob = -policy_out.log_prob(actions).sum(-1, keepdim=False)
        
        policy_loss = torch.mean(weight * log_prob)
